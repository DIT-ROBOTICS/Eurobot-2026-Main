...
# 
# This script creates a tmux session with 3 or 4 panes:
...
#   Right-Bottom (small): micro_ros_agent  (only if ENABLE_MICRO_ROS=true)
#
...
# Default: micro_ros_agent is disabled (moved to separate repo)
ENABLE_MICRO_ROS="${ENABLE_MICRO_ROS:-false}"
...
# Source micro_ros workspace if it exists (only when enabled)
MICRO_ROS_SOURCE=""
if [ "$ENABLE_MICRO_ROS" = "true" ] && [ -f "$HOME/micro_ros_ws/install/setup.bash" ]; then
...
launch() {
    ...
    echo "============================================================="
    if [ "$ENABLE_MICRO_ROS" = "true" ]; then
        echo "  micro_ros_agent: ENABLED"
    else
        echo "  micro_ros_agent: DISABLED (set ENABLE_MICRO_ROS=true to enable)"
    fi
    echo "============================================================="
    ...
    if [ "$ENABLE_MICRO_ROS" = "true" ]; then
        ...
        # --- Send commands to each pane ---
        # Pane layout after splits (3 panes):
        #   0 = Left (bt_engine)
        #   1 = Right-Top (startup)
        #   2 = Right-Bottom (mock tester)
